...
class Login {
	...
	*/
	public function checkUserLogin($username, $password) {
		...
		$sql = 'SELECT id,first_name, member_redirect, member_email FROM intervenant WHERE member_email = ? AND member_password = ?';
		...
		if ($stmt = $this->conn->prepare($sql)) {
			...
			$stmt->execute();
			$stmt->bind_result($user_id,$first_name, $redir, $username);
			if($stmt->fetch()) {
				// Set some sessions 
