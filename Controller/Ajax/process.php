...
if (isset($_POST['regUser']) && isset($_POST['regPass']) && isset($_POST['regEmail']) && isset($_POST['regCaptcha'])) {
	$username = strip_tags($_POST['regUser']);
	$password = strip_tags(hash_pbkdf2("sha256", $_POST['regPass'], "salt", 1000, 20));
	$email = strip_tags($_POST['regEmail']);
	$captcha = $_POST['regCaptcha'];
	$userPassword = strip_tags($_POST['regPass']);
	echo $login->registerUser($username, $password, $email, $captcha, $userPassword);
}
