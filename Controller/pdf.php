...
class pdf {
    ...
    public function getDossierPdf($id) {
        ...
        $id_intervenant=$Dossier->get("id_intervenant",$id);
        $name=$User->get("first_name", $id_intervenant)." ".$User->get("last_name", $id_intervenant);
        $title=$Dossier->get("title",$id);
        $year=$Dossier->get("year",$id);

        // set document information
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetAuthor('Masset Eliot');
        ...
        if($User->get("ss", $id_intervenant)!=0)
            $num_id.="Numéro de Sécurité Social : ".$User->get("ss", $id_intervenant)." <br />";
        if($User->get("siret", $id_intervenant)!=0)
            $num_id.="Numéro de SIRET : ".$User->get("siret", $id_intervenant)." <br />";
        // Set some content to print
        $html = '<div style="text-align: center;"><u><h1>Contrat de Vacation</h1></u></div><br />
...
et<br /><br />
<b>'.$User->get("civilite", $id_intervenant).' '.$User->get("last_name", $id_intervenant).' '.$User->get("first_name", $id_intervenant).'</b>, né.e le '.implode('/',array_reverse  (explode('-',$User->get("born_date", $id_intervenant)))).' en '.$User->get("born_country", $id_intervenant).', de nationalité '.$User->get("nationality", $id_intervenant).', domicilié.e au '.$User->get("address", $id_intervenant).', '.$User->get("CP", $id_intervenant).' '.$User->get("city", $id_intervenant).' ( '.$User->get("live_country", $id_intervenant).' ) <br />
<br />
'.$num_id.'<br />
<br />
d’autre part,<br />
...
'.$User->get("civilite", $id_intervenant).' '.$User->get("last_name", $id_intervenant).' '.$User->get("first_name", $id_intervenant).' </b>est engagé.e en qualité d’intervenant.e extérieur.e au titre de l’article 6 sexies de la loi N°84-16 du 11 janvier 1984 modifiée.<br /><br />
...
foreach ($interventions as $inter) {
    ...
    $html.='<b>'.$User->get("civilite", $id_intervenant).' '.$User->get("last_name", $id_intervenant).' '.$User->get("first_name", $id_intervenant).'</b> assurera un maximum de '.$hours.' heures d’interventions portant sur '.$title_inter.' ( '.$cursus.' ) dans le cadre du programme de '.$type.' organisé par l’ENSP.<br /> <br /> 
    ';
}
$html.='
...
Pour ses interventions, <b>'.$User->get("civilite", $id_intervenant).' '.$User->get("last_name", $id_intervenant).' '.$User->get("first_name", $id_intervenant).'</b> percevra après service fait une rémunération de '.$price.' € brut, selon les tarifs fixés par délibération du Conseil d’Administration le 18 Novembre 2016.<br />
...
if($has_a_FC)
    ...
    <b>'.$User->get("last_name", $id_intervenant).' '.$User->get("first_name", $id_intervenant).'</b> assurera ses interventions en formation continue, selon les modalités suivantes : <br/><br />
